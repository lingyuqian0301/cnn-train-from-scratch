...
class DashboardRenderer:
    ...
    def render_roi_indicator(self, frame: np.ndarray, top_margin: int, bottom_margin: int) -> np.ndarray:
        annotated = frame.copy()
        h, w = frame.shape[:2]
        y1 = max(0, top_margin)
        y2 = max(y1 + 1, h - max(0, bottom_margin))
        cv2.rectangle(annotated, (0, y1), (w - 1, y2 - 1), (255, 255, 0), 2)
        return annotated
    ...
    def render_detections(self, frame: np.ndarray, detections: list) -> np.ndarray:
        ...
        for det in detections:
            ...
            cv2.putText(annotated, text, (x1 + 2, max(y1 - 6, 12)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
        return annotated
...
class RealTimeClassificationApp:
    ...
    def run(self):
        ...
        while True:
            ...
            if self.config.enable_object_detection:
                h, w = frame.shape[:2]
                y1 = max(0, self.config.roi_top_margin)
                y2 = max(y1 + 1, h - max(0, self.config.roi_bottom_margin))
                roi = frame[y1:y2, 0:w]
                if self.config.debug_show_roi_window:
                    cv2.imshow('ROI', roi)
                ...
                for cnt in contours:
                    ...
                    px = max(0, x - self.config.bbox_padding)
                    py = max(0, y - self.config.bbox_padding)
                    px2 = min(w - 1, x + bw + self.config.bbox_padding)
                    py2 = min(y2 - y1 - 1, y + bh + self.config.bbox_padding)
                    boxes.append((px, py, px2, py2))
                boxes = sorted(boxes, key=lambda b: (b[2]-b[0])*(b[3]-b[1]), reverse=True)[:self.config.max_objects_per_frame]
                detections = []
                first_logged = False
                for (rx1, ry1, rx2, ry2) in boxes:
                    gx1, gy1, gx2, gy2 = rx1, ry1 + y1, rx2, ry2 + y1
                    crop = frame[gy1:gy2, gx1:gx2]
                    pred_class, conf = self.inference_engine.predict_frame(crop)
                    if pred_class is None:
                        pred_class = "unknown"
                        conf = 0.0
                    instr = self.sorting_simulator.generate_instruction(pred_class, conf)
                    bin_color = self.sorting_simulator.get_bin_color(pred_class) if not instr.requires_manual_review else (128, 128, 128)
                    detections.append({
                        "box": [gx1, gy1, gx2, gy2],
                        "label": pred_class,
                        "confidence": conf,
                    ...
                    self.update_statistics(conf, not instr.requires_manual_review)
                    if self.config.enable_logging and not first_logged and (time.time() - last_log_time) >= log_interval:
                        self.logger.log_classification(
                            predicted_class=pred_class,
                            confidence=conf,
                            bin_location=instr.bin_location,
                            verification_status=self.sorting_simulator.get_verification_status(conf)[0],
                            manual_review=conf < 0.5
                        )
                        last_log_time = time.time()
                        first_logged = True
                if self.config.show_roi_indicator:
                    frame = self.dashboard.render_roi_indicator(frame, self.config.roi_top_margin, self.config.roi_bottom_margin)
                frame = self.dashboard.render_detections(frame, detections)
                if len(detections) > 0:
                    primary = max(detections, key=lambda d: d.get("confidence", 0.0))
                    pc = primary.get("label", "")
                    cf = float(primary.get("confidence", 0.0))
                    vs, sc = self.sorting_simulator.get_verification_status(cf)
                    bc = self.sorting_simulator.get_bin_color(pc)
                    instr = self.sorting_simulator.generate_instruction(pc, cf)
                    frame = self.dashboard.render_inference_info(frame, pc, cf, vs, sc)
                    if self.config.enable_sorting and not instr.requires_manual_review:
                        frame = self.dashboard.render_sorting_info(frame, instr.instruction_text, instr.bin_location, bc)
            else:
                predicted_class, confidence = self.inference_engine.predict_frame(frame)
                if predicted_class is not None:
                    instruction: SortingInstruction = self.sorting_simulator.generate_instruction(predicted_class, confidence)
            ...
            if self.config.display_fps:
                cv2.putText(frame, f"FPS: {fps:.1f}", (20, frame.shape[0] - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
    ...
    def shutdown(self):
        ...
        try:
            ...
            pass
        try:
            if self.config.enable_logging and self.sorting_simulator is not None:
                self.sorting_simulator.save_log()
                logger.info("Sorting log saved")
        except Exception:
            pass
