...
def main():
    """Test the sorting simulator"""
    print("=" * 60)
    print("SORTING SIMULATOR TEST")
    print("=" * 60)
    
    simulator = SortingSimulator(max_bin_capacity=10)
    
    # Test cases
    test_cases = [
        ("plastic", 0.85),   # High confidence
        ("glass", 0.65),     # Medium confidence
        ("metal", 0.45),     # Low confidence
        ("paper", 0.92),     # High confidence
        ("biological", 0.55), # Medium confidence
    ]
    
    print("\n[1] Generating sorting instructions...")
    for waste_type, confidence in test_cases:
        instruction = simulator.generate_instruction(waste_type, confidence)
        print(f"\n  Waste: {waste_type}, Confidence: {confidence:.0%}")
        print(f"  Instruction: {instruction.instruction_text}")
        print(f"  Bin: {instruction.bin_location}")
        print(f"  Manual Review: {instruction.requires_manual_review}")
    
    print("\n[2] Statistics:")
    stats = simulator.get_statistics()
    print(f"  Total Instructions: {stats['total_instructions']}")
    print(f"  Auto Sorted: {stats['auto_sorted']}")
    print(f"  Needs Verification: {stats['needs_verification']}")
    print(f"  Manual Reviews: {stats['manual_reviews']}")
    ...
    print("\n[5] Testing bin capacity warning...")
    # Fill a bin to capacity
    for i in range(12):
        simulator.generate_instruction("plastic", 0.90)
    
    print("\n✅ Sorting Simulator test complete!")
    print("=" * 60)
